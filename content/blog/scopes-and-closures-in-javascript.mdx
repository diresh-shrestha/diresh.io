...
description: "An in-depth look at scopes and closures in JavaScript"
excerpt: "An in-depth look at scopes and closures in JavaScript"
tags: ["JavaScript"]
---
...
Modern way of writing modules is to have each module in their own separate file, `export` them and subsequently `import` them when we need them. The modern way of writing modules is to have each module in their own separate file, `export` them and subsequently `import` them when we need them. Let's look at an example where we have three separate files `foo.js`, `bar.js`, and `index.js`.
